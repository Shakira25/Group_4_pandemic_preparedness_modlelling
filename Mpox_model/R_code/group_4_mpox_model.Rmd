...
base_params <- c(
  beta_s = (0.09 * 1.5),  # Transmission rate via the sexual route
  beta_ns = 0.09,         # Transmission rate via other non-sexual routes
  gamma_I = 0.7,              # Recovery rate for those who were infectious and remained in the general population
  sigma = 0.01,                # Detection rate
  gamma_Id = 1.4,             # Recovery rate for those who were detected
  rho_I = 0.02,                # Mortality rate for the infected people in the community
  rho_Id = 0.01               # Mortality rate for the infected people that were detected and isolated
)
...
lower_bounds <- c(beta_ns = 0.05, sigma = 0.001) #, rho_Id = 0.0001)
upper_bounds <- c(beta_ns = 0.5, sigma = 0.05)#, rho_Id = 0.05)

# Initial guesses for the parameters to be estimated
initial_guesses <- c(beta_ns = 0.09, sigma = 0.01)
...
projection_df$I = projection_df$I_s + projection_df$I_ns
#Plot the projection with dates on the x-axis
#print(
#  plot(projection_date_vector, projection_df$Id, type = "l", col = "red", ylab = "Projected Detected #Infected Humans (Id)", xlab = "Date", xaxt = "n")
#)
...
#plot(projection_df$I, col = "red", pch = 16, type="b")
plot(date_vector, observed_data, col = "blue", pch = 16, type="b")  # Observed data only goes up to current time
axis.Date(1, at = projection_date_vector, format = "%b %Y")
