...
base_params <- c(
  ...
  sigma = 0.5,            #Detection rate
  gamma_Id = 0.056,        # Recovery rate for thise who were detected
  rho_I = 0.007,           # Mortality rate for the infected people in the community
  rho_Id = 0.0065          # Mortality rate for the infected people that were detected and isolated
...
run_model <- function(params) {
  model <- function(t, state, parameters) {
    with(as.list(c(state, parameters)), {
      dS <- -beta * S * I*Id
      dI <- beta * S * I*Id - gamma_I * I - sigma*I - rho_I*I
