...
base_params <- c(
  beta = 0.25,    # Transmission rate
  gamma_I = 0.07,    # Recovery rate for general infectious population
...
mpox_model <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    ...
    N <- S + I + Id + R
    dS <- -beta * S * I/N
    dI <- beta * S * I/N - gamma_I * I - sigma * I
...
# Define base parameters
base_params <- c(
  beta_s = 0.25,         # Transmission rate via the sexual route
  beta_ns = 0.06,        # Transmission rate via other non-sexual routes
  gamma_I = 0.07,         # Recovery rate for those who were infectious and remained in the general population
  sigma = 0.01,          # Detection rate
...
# Define base parameters
base_params <- c(
  beta_s = 0.02068134,         # Transmission rate via the sexual route
  beta_ns = 0.06,        # Transmission rate via other non-sexual routes
  gamma_I = 0.07,         # Recovery rate for those who were infectious and remained in the general population
  sigma = 0.01,          # Detection rate
  gamma_Id = 0.14,       # Recovery rate for those who were detected
