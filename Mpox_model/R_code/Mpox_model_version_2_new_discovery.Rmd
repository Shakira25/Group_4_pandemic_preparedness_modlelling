...
base_params <- c(
  ...
  sigma = 0.4,       # Detection rate
  gamma_Id = 1.4   # Recovery rate for detected population
)

# Define initial state
initial_state <- c(
  S = 105e3,    # Susceptible (population at risk and not the total population of DRC)
  I = 20,     # Infectious
...
lower_bounds <- c(beta=0.1, sigma = 0.05) #, gamma_I=0.1)
upper_bounds <- c(beta=2, sigma = 1) #,gamma_I=1)
...
# Recreate the base parameters with the estimated beta and sigma
params <- base_params
params["beta"] <- estimated["beta"]
params["sigma"] <- estimated["sigma"]
#params["gamma_I"] <- estimated["gamma_I"]
#params["gamma_Id"] <- 2*estimated["gamma_I"]
...
# Define base parameters 
base_params <- c(
  beta = 1.1852452,         # Transmission rate 
  gamma_I = 0.7,        # Recovery rate for those who were infectious and remained on the general population
  sigma = 0.3692279,            #Detection rate
  gamma_Id = 1.4,        # Recovery rate for thise who were detected
  rho_I = 0.01,           # Mortality rate for the infected people in the community
  rho_Id = 0.005          # Mortality rate for the infected people that were detected and isolated
...
# Define base parameters
base_params <- c(
  beta_s = (1.1852452 * 1.5),  # Transmission rate via the sexual route
  beta_ns = 1.1852452,         # Transmission rate via other non-sexual routes
...
initial_state <- c(
  S = 105e3,    # Susceptible
  I_s = 11,     # Infectious via the sexual route
  I_ns = 9,     # Infectious via other routes
  Id = 2,       # Detected population
  R = 0,        # Recovered
  D = 0         # Dead
)
...
# Define observed data (mock data as an example)
observed_data <- output$Detected  # Replace this with actual observed data
...
# Define lower and upper bounds for the parameters
lower_bounds <- c(beta_ns = 0.1, sigma = 0.001, rho_Id = 0.001)
upper_bounds <- c(beta_ns = 3, sigma = 0.2, rho_Id = 0.05)

# Initial guesses for the parameters to be estimated
initial_guesses <- c(beta_ns = 1.1852452, sigma = 0.01, rho_Id = 0.005)
